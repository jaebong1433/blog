package me.jaebong.springbootdeveloper.config;

import lombok.RequiredArgsConstructor;
import me.jaebong.springbootdeveloper.service.UserDetailService;
...
public class WebSecurityConfig {
    ...
    //스프링 시큐리티 기능 비활성화 인증,인가 서비스를 모든 곳에 모두 적용하지 않음
    @Bean
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception{
        return http
                ...
                //특정 요청과 일치하는 url에 대한 액세스를 설정, 누구나 접근이 가능하게 설정
                .requestMatchers("/login", "/signup", "/user").permitAll()

                //웨에서 설정한 url이외의 요청에 대해서 설정, 별도의 인가는 필요하지 않지만 인증이 접근할 수 있음
